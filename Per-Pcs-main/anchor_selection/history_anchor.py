...
with open(args.candidate_path, 'rb') as f:
    anchor_candidate = pickle.load(f)

with open('./prompt/prompt.json', 'r') as f:
    prompt_template = json.load(f)

# Step 1: Load the DeBERTa-v3-base tokenizer and model
tokenizer = DebertaV2Tokenizer.from_pretrained('/inspire/hdd/global_user/zhangweinan-24046/deberta-v3-large')
model = DebertaV2Model.from_pretrained('/inspire/hdd/global_user/zhangweinan-24046/deberta-v3-large').cuda()
...
for user in tqdm(anchor_candidate.values()):
    ...
    for samples in user:
        ...
        elif args.task_name == 'pwab_pos':
            ids.append(str(samples['id']))
            visible_history_list.append(samples)
            samples['tool_call'] = samples['output']['tool_call']
        else:
            ids.append(str(samples['id']))
            visible_history_list.append(samples)
