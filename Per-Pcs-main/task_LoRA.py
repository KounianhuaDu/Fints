...
args.train_data_path = f"../pa_back/data/{task_name}/processed/remain_train.pkl"
...
def main(args):
    ...
    with open(args.train_data_path, 'rb') as f:
        train = pickle.load(f)
        train_user_data = list(train.values())
        print(len(train_user_data))
    ...
    if not args.infer:
        ...
        for epoch in trange(args.epochs):
            data_list = random.sample(process_train_data(train_user_data, args), 10000)
            print(len(data_list))
