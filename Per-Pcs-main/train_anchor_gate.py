...
def get_args_parser():
    ...
    # Dataset parameters
    # parser.add_argument("--test_data_path", default="/afs/crc.nd.edu/user/z/ztan3/Private/LoRA-composition/LaMP_data-final/movie/user_anchor_candidate.json", type=str, help="dataset path")
    
    parser.add_argument("--output_dir", default="./output/news_headline/Anchor_PEFT/gate", help="path where to save, empty for no saving")
    ...
    parser.add_argument("--w_gate", type=bool, default=True, help="whether to user gate, recommend TRUE!!")
    
    parser.add_argument("--anchor_path", type=str, default='./output/news_headline/Anchor_PEFT/LoRA', help="resume lora from checkpoint")
    parser.add_argument("--anchor_idx_path", type=str, default='./anchor_selection/news_headline/anchor_user_idx.pt', help="resume lora from checkpoint")
...
def process_train_data(user, k):
    ...
    if args.task_name == 'news_headline':
        ...
        full_prompt = prompt_template[args.task_name]['OPPU_full'].format(**user)
        if k > 0:
            history_string = get_his(task_name, str(user['id']), k, ranked_profile)
            prompt = history_string + "\n" + prompt
            full_prompt = history_string + "\n" + full_prompt
        prompt = prompt_template[args.task_name]['OPPU_input'].format(**user)
        full_prompt = prompt_template[args.task_name]['OPPU_full'].format(**user)
...
def main(args):
    ...
    anchor_idx = torch.load(args.anchor_idx_path, weights_only=False)

    with open(args.test_data_path, 'rb') as f:
        all_user_data = list(pickle.load(f).values())
    ...
    # for idx, user in tqdm(enumerate(all_user_data), total=len(all_user_data)):
    for idx in tqdm(range(len(anchor_idx))):
        ...
        lora_path = os.path.join(args.anchor_path, f'user_{user[0]["user_id"]}/lora_ckpt_{args.k}_{args.remain_rate}.pt')
        ...
        # if args.gate_reset:
        model.reset_gate_parameters()

        model.set_gate_trainable()
        ...
        torch.save(model.gate_state_dict(), os.path.join(user_out_dir, f'gate_ckpt_{args.k}_{args.remain_rate}.pt'))
