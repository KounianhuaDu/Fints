...
class BlockOutputWrapper(t.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.add_attention_activations is not None:
            original_attn_output = self.block.self_attn.activations
            if original_attn_output is not None:
                augmented_attn = add_vector_from_position(
                    matrix=original_attn_output,
                ...
                mlp_input = self.post_attention_layernorm(augmented_attn + args[0])
        ...
        elif self.add_mlp_activations is not None:
            attn_output = self.block.self_attn.activations
            if attn_output is not None:
                mlp_input = self.post_attention_layernorm(attn_output + args[0])
                mlp_output = self.block.mlp(mlp_input)
                
                augmented_mlp = add_vector_from_position(
                    matrix=mlp_output,
                    vector=self.add_mlp_activations,
                    position_ids=kwargs["position_ids"],
                    from_pos=self.from_position,
        ...
        # self.mlp_out_unembedded = self.unembed_matrix(self.norm(self.mlp_output))
    ...
    def add_attention(self, activations):
        self.add_attention_activations = activations

    def add_mlp(self, activations):
...
class LlamaWrapper:
    ...
    def get_mlp_activations(self, layer):
        attn_output = self.model.model.layers[layer].block.self_attn.activations
        if attn_output is None:
            raise ValueError("Activations not found! Run get_logits() to get activations.")
        
        if hasattr(self.model.model.layers[layer], 'mlp_output'):
            return self.model.model.layers[layer].mlp_output
        else:
            raise ValueError("Internal decoding not enabled. No mlp output.")

    def set_add_activations(self, layer, activations):
        self.model.model.layers[layer].add(activations)
        
    def set_add_attention_activations(self, layer, activations):
        self.model.model.layers[layer].add_attention(activations)

    def set_add_mlp_activations(self, layer, activations):
        self.model.model.layers[layer].add_mlp(activations)
